...
def search_employee(request):
    ...
    # Search by employee_name (case insensitive) excluding deleted employees
    employees = Employee.objects.filter(employee_name__icontains=query).exclude(employee_id__in=deleted_ids)

    if not employees.exists():
        return JsonResponse({"success": False, "message": "No matching active employee found."})

    # Prepare list of employee dicts
    employee_list = [
        {
